...
class SDFBasedLiquid(TFModel):
    ...
    def __init__(self):
        ...
        self.initial_density_data[:, size[-2] * 3 // 8 : size[-2] * 5 // 8 - 0, size[-1] * 3 // 8 : size[-1] * 5 // 8, :] = 1
        ...
        self.liquid = world.SDFLiquid(state_domain=domain, density=self.initial_density_data, velocity=self.initial_velocity_data, gravity=-10.0, distance=self.distance)
        ...
        self.loss = l2_loss(self.state_out.sdf - self.state_in.sdf)
    ...
    def step(self):
        ...
        if self.current_loss < 0.1 or self.steps > 100:
            self.steps = 0
            self.world_steps += 1
