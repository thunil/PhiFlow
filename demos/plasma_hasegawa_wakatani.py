...
class PlasmaSim(App):
        ...
        plasma = self.plasma = world.add(
            PlasmaHW(
                Domain(
                    resolution,
                    box=box[0:N, 0:N, 0:N],
                    boundaries=(PERIODIC, PERIODIC, PERIODIC)  # Each dim: OPEN / CLOSED / PERIODIC
                ),
                density=np.ones(shape=shape),
                omega=np.ones(shape=shape),
                phi=np.random.uniform(low=0, high=1, size=shape)
            ),
    ...
    def action_reset(self):
        ...
        self.plasma.omega = np.random.uniform(low=0, high=1, size=shape)
        self.plasma.phi = np.random.uniform(low=0, high=1, size=shape)
